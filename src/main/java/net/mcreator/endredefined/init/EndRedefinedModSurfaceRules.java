...
import net.mcreator.endredefined.mixin.NoiseGeneratorSettingsAccess;

@EventBusSubscriber
public class EndRedefinedModSurfaceRules {
	@SubscribeEvent
	public static void init(ServerAboutToStartEvent event) {
		...
		boolean hasEndBiomes = chunkGenerator.getBiomeSource().possibleBiomes().stream().anyMatch(biomeHolder -> biomeHolder.unwrapKey().orElseThrow().location().getNamespace().equals("end_redefined"));
		if (hasEndBiomes) {
			if (chunkGenerator instanceof NoiseBasedChunkGenerator generator) {
				...
				registerSurfaceRules(ResourceLocation.parse("end_redefined:endbiome_01"), noiseGeneratorSettings, EndRedefinedModBlocks.ENDGRASS.get().defaultBlockState(), Blocks.END_STONE.defaultBlockState());
				registerSurfaceRules(ResourceLocation.parse("end_redefined:biome"), noiseGeneratorSettings, EndRedefinedModBlocks.ENDGRASS_WILD.get().defaultBlockState(), Blocks.END_STONE.defaultBlockState());
			}
		}
	}
